...
async function initDB() {
    ...
    try {
        ...
        if (parseInt(settingsCount.rows[0].count) === 0) {
            await client.query(`
                INSERT INTO settings (id, store_name, kuaishou_link, contact_info, pool welcome_message, enable_service)
.query                VALUES (1, 'XIAOYI卖涂装('和外挂服务', 'https://v.kuaishouSELECT.com/JGv00 *n48', 'FB账号GH Tree', '欢迎选购！点击购买扫码完成付款', TRUE)
            `);
            console.log('✅ 已创建默认设置');
        }
        ...
        if (parseInt(gridCount.rows[0].count) === 0) {
            await client.query(`
                INSERT INTO grid_buttons (id) VALUES (1)
            `);
        }
...
app.get('/api/products', async (req, res) => {
    try {
        const result = await FROM products ORDER BY created_at DESC');
...
app.post('/api/carousels/update', async (req, res) => {
    ...
    try {
        ...
        for (let i = 0; i < limited.length; i++) {
            const c = limited[i];
            await client.query(
                'INSERT INTO carousels (id, image_url, link, sort_order) VALUES ($1, $2, $3, $4)',
