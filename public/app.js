class cpmcylone {
    ...
    bindEvents() {
        ...
        // 为自定义场数输入框添加事件监听
        const customWins = document.getElementById('custom-wins');
        const customLosses = document.getElementById('custom-losses');
        
        if (customWins) {
            customWins.addEventListener('input', () => this.validateStatsInput(customWins));
        }
        
        if (customLosses) {
            customLosses.addEventListener('input', () => this.validateStatsInput(customLosses));
        }
    ...
    validateStatsInput(inputElement) {
        const value = parseInt(inputElement.value);
        if (isNaN(value) || value < 0) {
            inputElement.value = '';
            this.showStatus('warning', '请输入有效的正整数', 'clone-status');
            return false;
        }
        return true;
    }
    ...
    displayAccountInfo(accountData) {
        ...
        // 显示成功场数和失败场数
        const wins = accountData.wins || accountData.totalWins || accountData.victories || 0;
        const losses = accountData.losses || accountData.totalLosses || accountData.defeats || 0;
        document.getElementById('account-wins').textContent = this.formatNumber(wins);
        document.getElementById('account-losses').textContent = this.formatNumber(losses);
        
        // 设置自定义场数输入框的默认值为当前场数
        const customWins = document.getElementById('custom-wins');
        const customLosses = document.getElementById('custom-losses');
        if (customWins) customWins.value = wins;
        if (customLosses) customLosses.value = losses;
    ...
    logout() {
        ...
        document.getElementById('account-wins').textContent = '--';
        document.getElementById('account-losses').textContent = '--';
        
        // 清空自定义场数输入框
        const customWins = document.getElementById('custom-wins');
        const customLosses = document.getElementById('custom-losses');
        if (customWins) customWins.value = '';
        if (customLosses) customLosses.value = '';
    ...
    async cloneAccount() {
        ...
        // 获取自定义场数
        const customWins = document.getElementById('custom-wins').value.trim();
        const customLosses = document.getElementById('custom-losses').value.trim();
        ...
        // 如果用户输入了自定义场数，使用用户的值
        if (customWins !== '') {
            const winsValue = parseInt(customWins);
            if (isNaN(winsValue) || winsValue < 0) {
                this.showStatus('error', '请输入有效的成功场数（正整数）', 'clone-status');
                return;
            }
            wins = winsValue;
        }
        ...
        if (customLosses !== '') {
            const lossesValue = parseInt(customLosses);
            if (isNaN(lossesValue) || lossesValue < 0) {
                this.showStatus('error', '请输入有效的失败场数（正整数）', 'clone-status');
                return;
            }
            losses = lossesValue;
        }
        ...
        if (operationType === 'clone-to-new') {
            ...
            // 构建确认信息，包含场数修改信息
            let confirmMessage = `⚠️ 警告：这将完全覆盖目标账号的所有数据！\n\n` +
                              `目标账号: ${targetEmail}\n` +
                              `新Local ID: ${customLocalId}\n\n`;
            
            // 添加场数修改信息
            const currentWins = document.getElementById('account-wins').textContent;
            const currentLosses = document.getElementById('account-losses').textContent;
            
            if (wins !== null || losses !== null) {
                confirmMessage += `场数修改信息：\n`;
                confirmMessage += `  成功场数: ${currentWins} → ${wins !== null ? wins : currentWins}\n`;
                confirmMessage += `  失败场数: ${currentLosses} → ${losses !== null ? losses : currentLosses}\n\n`;
            }
            
            confirmMessage += `源账号车辆: ${document.getElementById('account-cars').textContent} 辆\n` +
            ...
            // 记录场数修改信息
            if (wins !== null) {
                this.addLog(`自定义成功场数: ${wins}`);
            }
            if (losses !== null) {
                this.addLog(`自定义失败场数: ${losses}`);
            }
            ...
            try {
                ...
                // 如果有自定义场数，添加到请求数据中
                if (wins !== null) {
                    requestData.customWins = wins;
                }
                if (losses !== null) {
                    requestData.customLosses = losses;
                }
                ...
                if (data.ok) {
                    ...
                    // 显示场数修改信息
                    if (wins !== null || losses !== null) {
                        this.addLog(`场数已修改: 成功${wins !== null ? wins : '不变'}, 失败${losses !== null ? losses : '不变'}`);
                    }
                    
                    this.addLog(`已克隆车辆: ${data.details?.carsCloned || '未知'} 辆`);
                    this.addLog(`总耗时: ${elapsedTime} 秒`);
                    
                    let statusMessage = `账号克隆成功！耗时 ${elapsedTime} 秒`;
                    if (wins !== null || losses !== null) {
                        statusMessage += `（已更新场数）`;
                    }
                    
                    this.showStatus('success', statusMessage, 'clone-status');
            ...
            // 构建确认信息，包含场数修改信息
            let confirmMessage = `⚠️ 确认修改当前账号Local ID？\n\n` +
            ...
            if (wins !== null || losses !== null) {
                confirmMessage += `同时修改场数统计：\n`;
                confirmMessage += `  成功场数: ${currentWins} → ${wins !== null ? wins : currentWins}\n`;
                confirmMessage += `  失败场数: ${currentLosses} → ${losses !== null ? losses : currentLosses}\n\n`;
            }
            
            confirmMessage += `此操作会更新所有车辆数据中的Local ID引用。`;
            ...
            try {
                ...
                if (data.ok) {
                    ...
                    // 显示场数修改信息
                    if (wins !== null || losses !== null) {
                        this.addLog(`场数已更新: 成功${wins !== null ? wins : '不变'}, 失败${losses !== null ? losses : '不变'}`);
                    ...
                    let statusMessage = `ID修改成功！耗时 ${elapsedTime} 秒`;
                    if (wins !== null || losses !== null) {
                        statusMessage += `（已更新场数）`;
                    }
    ...
    addLog(message) {
        ...
        else if (message.includes('成功场数') || message.includes('失败场数')) iconClass = 'fa-chart-bar';
