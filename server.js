...
app.post('/api/verify', (req, res) => {
    ...
    if (!accessKey || !username) {
        return res.json({ success: false, message: '请填写完整的验证信息' });
    }
    ...
    if (keyData.status === 'used') {
        ...
        if (keyData.boundUser === username) {
            console.log(`用户登录: ${username} 使用 ${keyData.type === 'hour' ? '小时卡' : '全功能卡'}`);
            return res.json({
                success: true,
                message: '用户验证成功',
                userType: 'user',
            ...
            console.log(`密钥已绑定其他用户: ${keyData.boundUser}`);
            return res.json({ success: false, message: '此密钥已绑定其他账号' });
        }
    } else {
        // 新密钥，绑定用户
        keyData.status = 'used';
        ...
        console.log(`新用户绑定: ${username} 绑定 ${keyData.type === 'hour' ? '小时卡' : '全功能卡'} ${keyData.key}`);
        return res.json({
            success: true,
            message: '用户绑定成功',
            userType: 'user',
...
app.get('/api/users', (req, res) => {
    // 从keysDatabase提取用户信息
    const users = keysDatabase
        .filter(key => key.status === 'used' && key.boundUser)
        .map(key => ({
            username: key.boundUser,
            key: key.key,
            cardType: key.type,
            created: key.bindTime,
            status: 'active'
        }));
...
app.get('/api/logs', (req, res) => {
    // 简单日志
    const logs = keysDatabase
        .filter(key => key.bindTime)
        .map(key => ({
            time: key.bindTime,
            user: key.boundUser,
            action: '绑定密钥',
            content: `用户 ${key.boundUser} 绑定了${key.type === 'hour' ? '小时卡' : '全功能卡'} ${key.key}`,
            ip: '127.0.0.1'
        }));
...
app.listen(PORT, () => {
    ...
    console.log(`✨ Version: 3.0.0 - Added key verification system`);
});
